...
stage_env () {
  ...
  git checkout .circleci/config.yml .circleci/deploy.sh .circleci/push.sh poetry.lock
  git add --all
  echo
  echo 'Done!'
  echo
...
makenew () {
  ...
  sed_delete README.rst '24,152d'
  sed_insert README.rst '24i' 'TODO'
  ...
  find_replace "s/makenew-python-app/${mk_slug}/g"
  find_replace "s/makenew_python_app/${mk_module}/g"

  git mv makenew_python_app ${mk_module}
